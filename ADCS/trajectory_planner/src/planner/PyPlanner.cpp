...
py::tuple PyPlanner::trajOptBeforePython(VECTOR_INFO_PY_FORM vecs_w_timePy,int N, TIME_FORM time_start, TIME_FORM time_end, py::array_t<double> x0Numpy, int bdotOn){
  vec x0 = numpyToArmaVector(x0Numpy);
  VECTOR_INFO_FORM vecs_w_time = vecsPy2Cpp(vecs_w_timePy);

  BEFORE_OUTPUT_FORM results = op.trajOptBefore(vecs_w_time, N, time_start, time_end, x0, bdotOn);
...
py::tuple PyPlanner::trajOptAfterPython(VECTOR_INFO_PY_FORM vecs_w_timePy,int N, TIME_FORM time_start, TIME_FORM time_end, ALILQR_OUTPUT_PY_FORM alilqrOutPy)
{
  VECTOR_INFO_FORM vecs_w_times = vecsPy2Cpp(vecs_w_timePy);
  ALILQR_OUTPUT_FORM alilqrOut = alilqrOutPy2Cpp(alilqrOutPy);
  AFTER_OUTPUT_FORM results = op.trajOptAfter(vecs_w_times, N, time_start, time_end, alilqrOut);
  py::tuple resultsPy = afterOutputCpp2Py(results);
  //return success
  return resultsPy;
}
...
bool PyPlanner::ilqrBreakPython(double grad, double LA, double dLA, double dlaZcount, double cmaxtmp, double iter,BREAK_SETTINGS_PY_FORM breakSettingsPy,int outer_iter, int ilqr_iter)
{
  op.current_ilqr_iter = ilqr_iter;
  op.current_outer_iter = outer_iter;
  BREAK_SETTINGS_FORM breakSettings_tmp = breakSettingsPy2Cpp(breakSettingsPy);
  return op.ilqrBreak(grad,LA,dLA,dlaZcount,cmaxtmp,iter,breakSettings_tmp);
}
