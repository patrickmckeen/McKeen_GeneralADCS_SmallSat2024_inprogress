...
void Satellite::add_prop_torq_py(py::array_t<double> prop_torq_in_py){
  vec3 prop_torq_in = numpyToArmaMatrix(prop_torq_in_py);
  plan_for_prop = 1;
  prop_torq = prop_torq_in;
}
...
vec Satellite::getConstraints(int k, int N, vec u, vec x, arma::vec3 sunECIvec) const
{
  ...
  if(k <= N-1 && k >= 0)
  {
    int ind = 0;
    if(useAVconstraint){
      ck.row(ind) = (dot(x.rows(avindex0(),avindex0()+2),x.rows(avindex0(),avindex0()+2))-AVmax*AVmax)*(180.0/datum::pi)*(180.0/datum::pi);//put in degrees to make the magnitude bigger so more compatible with other costs/etc in alg.
      ind++;
    }
...
tuple<vec,vec3> Satellite::dynamics(vec x, vec u, DYNAMICS_INFO_FORM dynamics_info) const
{
  ...
  vec3 variable_dist_torq = dist_on*(prop_torq_on*prop_torq*plan_for_prop+gen_dist_torq*plan_for_gendist);
  vec3 dist_torq = dist_on*gg_torq + variable_dist_torq;
  ...
  return std::make_tuple(res,variable_dist_torq);
}
