...
BEFORE_OUTPUT_FORM OldPlanner::trajOptBefore(VECTOR_INFO_FORM vecs_w_time,int N, TIME_FORM time_start, TIME_FORM time_end, vec x0, int bdotOn)
{
  // cout<<x0<<"\n";
  ...
  int traj_length = floor((time_end-time_start)*36525.0*24.0*3600.0/dt);
  cout<<"doing vec times\n";
  // vec t = get<0>(vecs_w_time);

  VECTOR_INFO_FORM vecs = findVecTimes(vecs_w_time,dt/(36525.0*24.0*3600.0),traj_length,time_start,time_end);
...
//  double penInit = this->penInit;//readJsonDouble(trajOptSettingsFile, "penInit");
  ...
  mat X = mat(sat.state_N(),traj_length).zeros();
  mat TQ = mat(3,traj_length).zeros();
  ...
  if(bdotOn==0 || sat.number_MTQ<3 || bdotOn>3)
  {
    ...
    if(verbose)
    {
     ...
     traj = OldPlanner::generateInitialTrajectory(dt,x0, U, vecs);
     assert(approx_equal(get<1>(traj),U,"abstol",1e-10));
     X = get<0>(traj);
  }
  else
  {
    std::tuple<TRAJECTORY_FORM,double> bdotout = OldPlanner::bdot(x0,dt,traj_length,vecs,costSettings_tmp,mu0);
    if(verbose)
    {
      cout<<"bdot attempted\n";
    }
    ...
    if (bdotOn == 2)
    {
      std::tuple<TRAJECTORY_FORM,double> sbdotout = OldPlanner::smartbdot(x0,dt,traj_length,vecs,costSettings_tmp,mu0,false);
      cout<<"smart bdot complete\n";
      traj = std::get<0>(sbdotout);
      mat u0 = get<1>(traj);
      cout<<size(get<0>(traj))<<" "<<size(get<1>(traj))<<"\n";
      TRAJECTORY_FORM traj2 = OldPlanner::generateInitialTrajectory(dt,x0, u0,vecs);//+diagmat(mean(abs(u0),1)*randval)*(randu(size(u0))-0.5), vecs);
    ...
    else if (bdotOn == 3)
    {
      ...
      traj = OldPlanner::generateInitialTrajectory(dt,x0, u0+diagmat(mean(abs(u0),1)*randval)*(randu(size(u0))-0.5), vecs);
    }
    U.cols(0,traj_length-1) = get<1>(traj);
    X = get<0>(traj);
    TQ = get<3>(traj);
    cout<<x0<<dt<<traj_length<<mu0<<X.has_nan()<<U.has_nan()<<"\n";
...
AFTER_OUTPUT_FORM OldPlanner::trajOptAfter(VECTOR_INFO_FORM vecs_w_time,int N, TIME_FORM time_start, TIME_FORM time_end, ALILQR_OUTPUT_FORM alilqrOut)
{
  ...
  VECTOR_INFO_FORM vecs_tvlqr = findVecTimes(vecs_w_time,dt_tvlqr/(36525.0*24.0*3600.0),traj_length,time_start,time_end);
  ...
  if(dt/dt_tvlqr > 1){
    double colMissing = max(0,int(Rset_tvlqr.n_cols) - int(int(Uset.n_cols)*dt/dt_tvlqr));
    if(verbose) {
      cout<<"colMiss: "<<colMissing<<"\n";
    }
    mat UsetLong = join_rows(repelem(Uset,1,dt/dt_tvlqr),mat(Uset.n_rows,colMissing).zeros());
  ...
  do{
      ...
      int traj_length_tmp = round((time_end_tmp-time_start_tmp)*36525.0*24.0*3600.0/dt_tvlqr);
      cout<<traj_length_tmp<<"\n";
      ...
      VECTOR_INFO_FORM vecs_tvlqr_tmp = findVecTimes(vecs_w_time,dt_tvlqr/(36525.0*24.0*3600.0),traj_length_tmp,time_start_tmp,time_end_tmp+0*dt_tvlqr/(36525.0*24.0*3600.0));
...
AFTER_OUTPUT_FORM OldPlanner::trajOpt(VECTOR_INFO_FORM &vecs,int N, TIME_FORM time_start, TIME_FORM time_end, vec x0, int bdotOn)
{
  ...
  cout<<"in c++\n";
  cout<<x0<<"\n";
  BEFORE_OUTPUT_FORM results = OldPlanner::trajOptBefore(vecs, N, time_start, time_end, x0, bdotOn);
  cout<<"past before\n";
  ...
  ALILQR_OUTPUT_FORM alilqrOut = OldPlanner::alilqr(dt,traj_init, vecs_dt, costSettings_tmp,alilqrSettings,true);
  cout<<"out of alilqr\n";
  ...
  cout<<"dist done\n";
  AFTER_OUTPUT_FORM results2 = OldPlanner::trajOptAfter(vecs, N, time_start, time_end, alilqrOut);
  cout<<"last bit done\n";
  return results2;
}
...
tuple<TRAJECTORY_FORM,double> OldPlanner::smartbdot(vec x0,double dt0,int N,VECTOR_INFO_FORM vecs,COST_SETTINGS_FORM costSettings_tmp,double mu,bool invert)
{
  ...
  vec uk = OldPlanner::smartbdot_rawmtq_finder(dt0,xk,nB2, ECIvk, ECIvkp1, satvk, Bbody,sbSettings);
  ...
  for(int k=1; k<N; k++)
  {
    ...
    //uk = -bdotgain*(-cross(newX(1:3,1),rotT(newX(end-3:end,1))*Bset(:,1)) + rotT(newX(end-3:end,1))*(Bset(:,2)-Bset(:,1))/dt0);
    ECIvk = ECIvec.col(k);
    ECIvkp1 = ECIvec.col(k+1);
    satvk = satvec.col(k);
    RmatT = rotMat(qk).t();
    Bbody = RmatT*Bk;
    uk = OldPlanner::smartbdot_rawmtq_finder(dt0,xk,nB2, ECIvk, ECIvkp1, satvk, Bbody,sbSettings);
...
vec OldPlanner::smartbdot_rawmtq_finder(double dt0, vec xk, double nB2,vec ECIvk, vec ECIvkp1, vec3 satvk, vec3 Bbody,SMARTBDOT_SETTINGS_FORM sbSettings){
  ...
  if(ek_is_3&&ekp1_is_3){
    ...
    vec3 bn = normalise(Bbody);//normalise(sat.Jcom*cross(xx,x2));
      ...
      //uk = cross(Bbody,sat.Jcom*(dampgain*(wk-dot(wk,x3)*x3) + quatgain*acos(norm_dot(ECIvkBody,satvk))*normalise(cross(ECIvkBody,satvk))))/nB2;
    uk.head(sat.number_MTQ) = (sat.mtq_ax_mat.t()*cross(Bbody,(dampgain*(sat.Jcom*wk) + velgain*(sat.Jcom*(wk-wkdes)) + quatgain*2.0*acos(as_scalar(qerr(0)))*qq)))/nB2;
...
tuple<cube, cube> OldPlanner::findK(double dt_tvlqr0, TRAJECTORY_FORM traj, VECTOR_INFO_FORM vecs, COST_SETTINGS_FORM costSettings_tmp)
{
  ...
  for(int k=N-2; k>=0; k--)
  {
    ...
    Kk = solve((lkuu + trans(Bqk)*Skp1*Bqk), (trans(Bqk)*Skp1*Aqk),solve_opts::likely_sympd+solve_opts::refine+solve_opts::equilibrate);//inv(R + trans(Bqk)*Skp1*Bqk)*(trans(Bqk)*Skp1*Aqk);//
...
ALILQR_OUTPUT_FORM OldPlanner::alilqr(double dt0,TRAJECTORY_FORM traj, VECTOR_INFO_FORM &vecs, COST_SETTINGS_FORM costSettings_tmp, ALILQR_SETTINGS_FORM alilqrSettings_tmp,bool isFirstSearch)
{
  ...
  double dLA = 0.0;
  double newLA = LA;
  REG_PAIR regs = make_tuple(regInit_tmp,regInit_tmp);
  ...
  for(int j = 0; j < maxOuterIter_tmp; j++)
  {
    ...
    regs = make_tuple(regInit_tmp,regInit_tmp);
...
tuple<BACKWARD_PASS_RESULTS_FORM, REG_PAIR> OldPlanner::backwardPass(double dt0,TRAJECTORY_FORM traj, VECTOR_INFO_FORM &vecs, AUGLAG_INFO_FORM auglag_vals,REG_PAIR regs, COST_SETTINGS_FORM *costSettings_tmp,REG_SETTINGS_FORM regSettings_tmp,bool useDist)
{
  ...
  while(k >= 0)
  {
    ...
    if(!reset){
      ...
      reset |= (min(eigs) <= -rho);
      if(verbose&&reset){
        cout<<"Qkuu has eigs<=-rho: "<<min(eigs)<<" < "<<-rho<<"\n";
        cout<<"k "<<k<<"\n";
        cout<<Qkuu<<"\n";
        cout<<Qku.t()<<"\n";
        // cout<<costJac.luu<<"\n";
        // cout<<trans(Bqk)*Pk*Bqk<<"\n";
        // cout<<Pk<<"\n";
        // cout<<Bqk<<"\n";
        // cout<<pk.t()<<"\n";
        // cout<<trans(cku)*Imuk*cku<<"\n";
        // cout<<mat(sum(get<0>(cnstrHess) % violcuu,2))<<"\n";
      }
      if(!reset){
        ...
        reset |= !solve(Kk,Qkuureg, Qkux,solve_opts::likely_sympd+solve_opts::refine+solve_opts::no_approx+solve_opts::equilibrate);//+solve_opts::no_approx);//+solve_opts::no_approx);//+solve_opts::equilibrate+solve_opts::refine+solve_opts::no_approx);
        if(verbose&&reset){
          cout<<"Solving Kk failed \n";
        }
        reset |= !solve(dk,Qkuureg, Qku,solve_opts::likely_sympd+solve_opts::refine+solve_opts::no_approx+solve_opts::equilibrate);//+solve_opts::no_approx);//+solve_opts::no_approx);//+solve_opts::equilibrate+solve_opts::refine+solve_opts::no_approx);
        if(verbose&&reset){
...
tuple<TRAJECTORY_FORM,double, REG_PAIR> OldPlanner::forwardPass(double dt0,TRAJECTORY_FORM traj, VECTOR_INFO_FORM &vecs, AUGLAG_INFO_FORM auglag_vals, BACKWARD_PASS_RESULTS_FORM BPresults, REG_PAIR regs, COST_SETTINGS_FORM *costSettings_tmp_ptr, REG_SETTINGS_FORM regSettings_tmp, LINE_SEARCH_SETTINGS_FORM lineSearchSettings_tmp,bool useDist)
{
  ...
  while((z<=beta1_tmp||z>beta2_tmp)||(newLA>=LA))
  {
    ...
    if(verbose){cout<<"ls iter, LA-nLA, exp, z,alph "<<lsiter<<" "<<LA-newLA<<" "<<exp<<" "<<z<<" "<<alph<<"\n";}
    lsiter++;
    alph /= 2.0;
  }
